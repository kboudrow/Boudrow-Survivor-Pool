...
function fmtEtCompactDateTime(value?: string | null) {
  if (!value) return '-'
...
function MyPoolsContent() {
  ...
  const compactDeadlineLabel =
    pool?.deadline_mode === 'rolling'
      ? 'Each game locks at kickoff'
      : `Sun ${formatEtTime(pool?.deadline_fixed)} · early games at kickoff`
              ...
              {!detailsLoading && pool && activeTab === 'picks' && (
                <>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="mb-3 rounded-lg border border-gray-200 bg-white p-2.5 sm:p-3">
                      <div className="flex flex-wrap items-start justify-between gap-2 sm:gap-3">
                        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
                          ...
                          <span className="rounded-full bg-slate-100 px-2 py-1 font-semibold sm:hidden">{compactDeadlineLabel}</span>
                          <span className="hidden rounded-full bg-slate-100 px-2 py-1 font-semibold sm:inline-flex">{deadlineLabel}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          {myEntries.length > 1 && (
                          ...
                          {draftSavedAt && (
                            <div className="text-xs font-medium text-emerald-700">
                              ...
                              <span className="hidden sm:inline">Saved {fmtDateTime(draftSavedAt)}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <details className="mt-2 rounded-md border border-slate-200 bg-slate-50 p-2 sm:mt-3">
                        <summary className="cursor-pointer text-xs font-semibold text-slate-800">
                          <span className="sm:hidden">Used teams ({usedTeamAbbrs.length})</span>
                          <span className="hidden sm:inline">Teams this entry already used ({usedTeamAbbrs.length})</span>
                        </summary>
                    ...
                    <section className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4">
                      <div className="mb-3 flex flex-wrap items-start justify-between gap-2 sm:mb-4 sm:gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-600 sm:mt-2 sm:text-sm">
                            <span className="sm:hidden">
                              Pick {picksAllowedForWeek(selectedPickWeek) === 1 ? 'one team' : 'two different teams'} for {selectedEntry ? entryLabelForMember(selectedEntry) : 'this entry'}. Saves automatically; editable until lock.
                            </span>
                        ...
                        <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-600">
                          <span className="sm:hidden">
                            {isTestMode && pool?.test_now_at ? `Test time: ${fmtEtCompactDateTime(pool.test_now_at)}` : selectedWeekCloseLabel}
                          </span>
                          <span className="hidden sm:inline">{selectedWeekCloseLabel}</span>
                        </div>
                      </div>
