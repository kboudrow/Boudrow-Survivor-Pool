...
begin
  ...
  left join (
    ...
    from public.pool_members group by pool_id
  ) pm on pm.pool_id = p.id
  left join (
    select pool_id, count(*) as draft_picks_count from public.pool_pick_drafts group by pool_id
  ) d on d.pool_id = p.id
  left join (
    select pool_id, count(*) as final_picks_count from public.pool_picks group by pool_id
  ) fp on fp.pool_id = p.id
  left join (
    select pool_id, count(*) as stats_rows_count from public.pool_member_stats group by pool_id
  ) s on s.pool_id = p.id
