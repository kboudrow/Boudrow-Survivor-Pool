...
begin
  ...
  left join (
    select owned.created_by, count(*) as pools_created, max(owned.created_at) as latest_pool_created_at
    from public.pools owned
    group by owned.created_by
  ) created on created.created_by = u.id
  left join (
    select memberships.profile_id, count(distinct memberships.pool_id) as pools_joined, count(*) as entries_count
    from public.pool_members memberships
